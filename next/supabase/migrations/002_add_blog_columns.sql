...
-- Backfill is_published from existing status column for any current rows
update public.posts set is_published = true where status = 'published' and is_published = false;

-- Fast index for the public listing query
create index if not exists posts_published_at_idx on public.posts (publish_at desc) where is_published = true;
