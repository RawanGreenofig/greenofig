...
/**
 * Job-posting management UI (list + create/edit/publish/delete). Shared by
 * /admin/careers and the head coach's /nutritionist/careers/postings —
 * both write through /api/admin/careers, which is gated to admin OR the
 * head coach. Reads job_postings directly (RLS allows both roles to read).
 */
...
export function JobPostingsManager({
  ...
  async function save(draft: Draft): Promise<{ error?: string }> {
    // Send the whole draft to the API. Server-side validates job_type,
    // stamps published_at, and audits create-vs-update.
  ...
  async function togglePublish(job: Job) {
    ...
    if (!res.ok) {
      console.error('[careers] toggle failed:', res.status)
      return
    }
    void reload()
  }
  ...
  async function remove(jobId: string) {
    ...
    if (!res.ok) {
      console.error('[careers] delete failed:', res.status)
      return
    }
    void reload()
  }
    ...
    <div className="px-4 md:px-8 py-6 md:py-8 max-w-screen-xl mx-auto space-y-6">
      <header className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          {subtitle && <p className="mt-2 text-sm md:text-base text-fg-2">{subtitle}</p>}
        </div>
        <button
          type="button"
            ...
            <div className="grid grid-cols-1 sm:grid-cols-6 gap-4">
              ...
              <div className="sm:col-span-3">
                <FormField label="Location">
                  <Input value={d.location ?? ''} onChange={(v) => update('location', v)} placeholder="Lawrenceville / Remote" />
                </FormField>
              </div>
            </div>
          </Section>
