 ...
 *
 * Secret-gated entry point for OpenClaw — Nutrition Coach Rawan's WhatsApp/Telegram
...
export async function POST(req: NextRequest) {
  ...
  const requestedBy = body.requestedBy ?? 'Nutrition Coach Rawan'
    ...
    switch (action) {
      ...
      case 'send_reminder': {
        ...
        const sent = await broadcast(service, {
          tiers: targetTiers,
          title: 'A note from Nutrition Coach Rawan',
          body: message,
          bodyAr: messageAr,
          category: 'reminder',
        })
